tinymce.PluginManager.add('toc', function (e) {
	...
	function r() {
		...
		t.disabled(e.readonly || !o()),
			e.on('LoadContent SetContent change', function () {
				t.disabled(e.readonly || !o())
			})
	}
	function i(e) {
	...
	function a(t) {
		...
		return (
			r.length &&
				/^h[1-9]$/i.test(t.headerTag) &&
				(r = r.filter(function (n, r) {
					return !e.dom.hasClass(r.parentNode, t.className)
				})),
			tinymce.map(r, function (e) {
				return e.id || (e.id = h()), {id: e.id, level: parseInt(e.nodeName.replace(/^H/i, ''), 10), title: f.text(e)}
			})
		)
	}
	function s(e) {
	...
	function u(e) {
		...
		for (o += l(e.headerTag, tinymce.translate('Table of Contents')), t = 0; t < c.length; t++) {
			...
			else for (n = u; n < r.level; n++) o += '<ul><li>'
			if (((o += '<a href="#' + r.id + '">' + r.title + '</a>'), i !== r.level && i)) for (n = r.level; n > i; n--) o += '</li></ul><li>'
			else (o += '</li>'), i || (o += '</ul>')
			u = r.level
		}
		return o
	...
	e.on('PreInit', function () {
		...
		d = {depth: r >= 1 && r <= 9 ? r : p.depth, headerTag: t(n.toc_header) ? n.toc_header : p.headerTag, className: n.toc_class ? e.dom.encode(n.toc_class) : p.className}
	}),
	...
	e.addCommand('mceInsertToc', function () {
		...
		e.addButton('toc', {tooltip: 'Table of Contents', cmd: 'mceInsertToc', icon: 'toc', onPostRender: r}),
		e.addButton('tocupdate', {tooltip: 'Update', cmd: 'mceUpdateToc', icon: 'reload'}),
		e.addContextToolbar(n, 'tocupdate'),
		e.addMenuItem('toc', {text: 'Table of Contents', context: 'insert', cmd: 'mceInsertToc', onPostRender: r})
})
