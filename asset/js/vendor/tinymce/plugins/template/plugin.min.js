tinymce.PluginManager.add('template', function (e) {
	...
	function n(t) {
		function n(t) {
			...
			a.url
				...
				: ((i = a.content), n(i)),
				r.find('#description')[0].text(t.control.value().description)
		}
		var r,
			i,
			s = []
		...
		tinymce.each(t, function (e) {
			s.push({selected: !s.length, text: e.title, value: {url: e.url, content: e.content, description: e.description}})
		}),
			(r = e.windowManager.open({
				...
				spacing: 10,
				items: [
					{type: 'form', flex: 0, padding: 0, items: [{type: 'container', label: 'Templates', items: {type: 'listbox', label: 'Templates', name: 'template', values: s, onselect: n}}]},
	...
	function a(t, n) {
		...
		;(n = o(n, 'template_replace_values')),
			(l = u.create('div', null, n)),
			(c = u.select('.mceTmpl', l)),
			c && c.length > 0 && ((l = u.create('div', null)), l.appendChild(c[0].cloneNode(!0))),
			s(u.select('*', l), function (t) {
	...
	e.addCommand('mceInsertTemplate', a),
		e.addButton('template', {title: 'Insert template', onclick: t(n)}),
		e.addMenuItem('template', {text: 'Template', onclick: t(n), context: 'insert'}),
		e.on('PreProcess', function (t) {
			var n = e.dom
			s(n.select('div', t.node), function (t) {
				n.hasClass(t, 'mceTmpl') &&
					(s(n.select('*', t), function (t) {
