...
;(jvm.$ = jQuery),
	...
	(jvm.NumericScale.prototype = {
		...
		getTicks: function () {
			var tick,
				...
				err = (m / span) * step,
				ticks = []
			for (0.15 >= err ? (step *= 10) : 0.35 >= err ? (step *= 5) : 0.75 >= err && (step *= 2), extent[0] = Math.floor(extent[0] / step) * step, extent[1] = Math.ceil(extent[1] / step) * step, tick = extent[0]; tick <= extent[1]; ) (v = tick == extent[0] ? this.clearMinValue : tick == extent[1] ? this.clearMaxValue : tick), ticks.push({label: tick, value: this.getValue(v)}), (tick += step)
			return ticks
		},
	}),
		...
		for (this.body.html(''), this.params.title && this.body.append(jvm.$('<div/>').addClass('jvectormap-legend-title').html(this.params.title)), this.body.append(inner), i = 0; i < ticks.length; i++) {
			switch (((tick = jvm.$('<div/>').addClass('jvectormap-legend-tick')), (sample = jvm.$('<div/>').addClass('jvectormap-legend-tick-sample')), this.series.params.attribute)) {
				...
				case 'r':
					jvm
						.$('<div/>')
						.css({'border-radius': ticks[i].value, border: this.map.params.markerStyle.initial['stroke-width'] + 'px ' + this.map.params.markerStyle.initial.stroke + ' solid', width: 2 * ticks[i].value + 'px', height: 2 * ticks[i].value + 'px', background: this.map.params.markerStyle.initial.fill})
						.appendTo(sample)
			}
	...
	(jvm.Map = function (params) {
		...
		;(this.canvas = new jvm.VectorCanvas(this.container[0], this.width, this.height)),
			...
			this.updateSize(),
			this.params.focusOn && ('string' == typeof this.params.focusOn ? (this.params.focusOn = {region: this.params.focusOn}) : jvm.$.isArray(this.params.focusOn) && (this.params.focusOn = {regions: this.params.focusOn}), this.setFocus(this.params.focusOn)),
	...
	(jvm.Map.prototype = {
			...
			var touchStartScale,
				...
				handleTouchEvent = function (e) {
					...
					'touchstart' == e.type && (lastTouchesLength = 0),
						1 == touches.length
							? (1 == lastTouchesLength && ((transXOld = map.transX), (transYOld = map.transY), (map.transX -= (touchX - touches[0].pageX) / map.scale), (map.transY -= (touchY - touches[0].pageY) / map.scale), map.applyTransform(), map.tip.hide(), (transXOld != map.transX || transYOld != map.transY) && e.preventDefault()), (touchX = touches[0].pageX), (touchY = touches[0].pageY))
							: 2 == touches.length && (2 == lastTouchesLength ? ((scale = Math.sqrt(Math.pow(touches[0].pageX - touches[1].pageX, 2) + Math.pow(touches[0].pageY - touches[1].pageY, 2)) / touchStartDistance), map.setScale(touchStartScale * scale, centerTouchX, centerTouchY), map.tip.hide(), e.preventDefault()) : ((offset = jvm.$(map.container).offset()), (centerTouchX = touches[0].pageX > touches[1].pageX ? touches[1].pageX + (touches[0].pageX - touches[1].pageX) / 2 : touches[0].pageX + (touches[1].pageX - touches[0].pageX) / 2), (centerTouchY = touches[0].pageY > touches[1].pageY ? touches[1].pageY + (touches[0].pageY - touches[1].pageY) / 2 : touches[0].pageY + (touches[1].pageY - touches[0].pageY) / 2), (centerTouchX -= offset.left), (centerTouchY -= offset.top), (touchStartScale = map.scale), (touchStartDistance = Math.sqrt(Math.pow(touches[0].pageX - touches[1].pageX, 2) + Math.pow(touches[0].pageY - touches[1].pageY, 2))))),
		...
		createRegions: function () {
			...
			for (key in this.mapData.paths)
				(region = new jvm.Region({map: this, path: this.mapData.paths[key].path, code: key, style: jvm.$.extend(!0, {}, this.params.regionStyle), labelStyle: jvm.$.extend(!0, {}, this.params.regionLabelStyle), canvas: this.canvas, labelsGroup: this.regionLabelsGroup, label: 'vml' != this.canvas.mode ? this.params.labels && this.params.labels.regions : null})),
